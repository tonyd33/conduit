...
import (
	...
	"github.com/tonyd33/conduit/internal/apiserver"
	"github.com/tonyd33/conduit/internal/controller"
	"github.com/tonyd33/conduit/internal/metrics"
...
func init() {
	...
	utilruntime.Must(conduitv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	var natsURL string
	var natsNKeyFile, natsCredentialsFile string
	var natsTLSCertFile, natsTLSKeyFile, natsTLSCAFile string
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "685acd81.mnke.org",
	...
	// Initialize NATS client if URL is provided
	var natsClient *natslib.Client
	var streamManager *natslib.StreamManager

	// Read sensitive credentials from environment variables for security
	// These should NEVER be passed as command-line flags
	natsUsername := os.Getenv("NATS_USERNAME")
	natsPassword := os.Getenv("NATS_PASSWORD")
	natsToken := os.Getenv("NATS_TOKEN")
	apiServerAPIKey := os.Getenv("API_SERVER_KEY")

	if natsURL != "" {
		setupLog.Info("Initializing NATS client", "url", natsURL)

		natsConfig := &natslib.ClientConfig{
			URL:             natsURL,
			Username:        natsUsername,
			Password:        natsPassword,
			Token:           natsToken,
			NKeyFile:        natsNKeyFile,
			CredentialsFile: natsCredentialsFile,
		...
		var err error
		natsClient, err = natslib.NewClient(context.Background(), natsConfig)
		if err != nil {
			setupLog.Error(err, "Failed to connect to NATS", "url", natsURL)
			os.Exit(1)
		}

		streamManager = natslib.NewStreamManager(natsClient)
		setupLog.Info("NATS client initialized successfully")

		// Add cleanup on shutdown
		defer func() {
			setupLog.Info("Closing NATS connection")
			if err := natsClient.Close(); err != nil {
				setupLog.Error(err, "Failed to close NATS connection")
			}
		}()
	} else {
		setupLog.Info("NATS URL not provided, running without NATS integration")
	}

	if err := (&controller.ExchangeReconciler{
		Client:        mgr.GetClient(),
		Scheme:        mgr.GetScheme(),
		Recorder:      mgr.GetEventRecorderFor("exchange-controller"),
	...
	// Setup webhooks
	if err := (&conduitv1alpha1.Exchange{}).SetupWebhookWithManager(mgr); err != nil {
	...
	metricsCollector := metrics.NewCollector(
		mgr.GetClient(),
		natsClient,
		30*time.Second, // Collect metrics every 30 seconds
	)
	...
	if apiServerAddr != "" {
		...
		apiServer := apiserver.NewServer(mgr.GetClient(), &apiserver.Config{
			Address: apiServerAddr,
			NATSURL: natsURL,
			APIKey:  apiServerAPIKey,
		})
		...
		setupLog.Info("API server address not provided, running without API server")
	}
