# Echo Exchange Example (TypeScript)

This is a simple example application that demonstrates how to use the Conduit TypeScript SDK to build an Exchange-compatible workload.
...
# Install the SDK (from the sdk/typescript directory)
cd ../../
npm install
npm run build
cd examples/echo
...
```bash
docker build -t echo-exchange-typescript:latest .
```
...
```bash
docker build -t <your-registry>/echo-exchange-typescript:latest .
docker push <your-registry>/echo-exchange-typescript:latest
```
...
kind: Exchange
metadata:
  name: echo-example-typescript
spec:
  image: "<your-registry>/echo-exchange-typescript:latest"
  resources:
    requests:
      memory: "128Mi"
      cpu: "100m"
...
- [x] Promise-based async/await message handling
